...
locals {
  ...
  vm_resource_labels = {
    "cpu"    = "${var.instance_resources.cores}"
...
resource "yandex_compute_instance" "this" {
  ...
  zone                      = each.value
  labels                    = local.vm_resource_labels
...
# Ожидание перед созданием снепшота
...
# Создание снепшота начального состояния дисков
...
# Если имя не будет уникальным, то использовать terraform apply -replace="random_string.bucket_name"
